const DUMMY_MEALS = [
  {
    id: 'm1',
    name: 'Single Kahuna',
    description:
      'One third pound burger with mustard, ketchup, pickles, mayonaise, lettuce, tomato and onion on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 11.5,
  },
  {
    id: 'm2',
    name: 'Single Kahuna with Cheese',
    description:
      'One third pound burger with cheese with mustard, ketchup, pickles, mayonaise, lettuce, tomato and onion on your choice of a Brioche roll or Chabatta bread.Served with huge helping of fries and your choice of beverage.',
    price: 12,
  },
  {
    id: 'm3',
    name: 'Barbecue Kahuna',
    description:
      'One third pound burger with bacon and barbeque sauce on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 12.5,
  },
  {
    id: 'm4',
    name: 'Big Kahuna',
    description:
      'Two third pound patties with double cheese with mustard, ketchup, pickles, mayonaise, lettuce, tomato and onion on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 14,
  },
  {
    id: 'm5',
    name: 'Chicken Kahuna',
    description:
      'Third pound chicken breast with bacon, lettuce, tomato, mayo, pickles and mustard on a delicious Brioche roll. Served with huge helping of fries and your choice of beverage.',
    price: 14,
  },
];

// change all single to double quotes. add double quote to all keys  and remove trailing comma from last item in hash.
const LIVE_MEALS = {
  m1: {
    name: 'Big Kahuna',
    description:
      'Two third pound patties with double cheese with mustard, ketchup, pickles, mayonaise, lettuce, tomato and onion on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 14,
  },
  m2: {
    name: 'Big Kahuna Barbecue',
    description:
      'Two third pound patties with bacon and barbeque sauce on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 14,
  },
  m3: {
    name: 'Little Kahuna',
    description:
      'One third pound burger with cheese with mustard, ketchup, pickles, mayonaise, lettuce, tomato and onion on your choice of a Brioche roll or Chabatta bread.Served with huge helping of fries and your choice of beverage.',
    price: 12,
  },
  m4: {
    name: 'Little Kahuna Barbecue',
    description:
      'One third pound burger with bacon and barbeque sauce on your choice of a Brioche roll or Chabatta bread. Served with huge helping of fries and your choice of beverage.',
    price: 12.5,
  },
  m5: {
    name: 'Kahuna Dawg',
    description:
      'One third pound brat with mustard, ketchup, sweet relish and onion on a soft Kings Hawaiian long roll. Served with huge helping of fries and your choice of beverage.',
    price: 11.5,
  },
  m6: {
    name: 'Kahuna Chicken',
    description:
      'Third pound chicken breast with bacon, lettuce, tomato, mayo and pickles on a delicious Brioche roll. Served with huge helping of fries and your choice of beverage.',
    price: 14,
  },
};
